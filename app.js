...
/**
 * Attaches event listeners to the grid items and buttons.
 *
...
function attachEventListeners() {
	document.querySelectorAll(".grid-item").forEach(function (gridItem) {
		...
		gridItem.addEventListener("mouseleave", function () {
			...
			if (!gridItem.dataset.modifiedName) {
				iframe.src = `./components/${gridItem.dataset.componentName}/screenshot.jpg`;
			} else {
				let modifiedComponentNameURL = gridItem.dataset.modifiedName.replace(/ /g, "-");
				iframe.src = `./components/${modifiedComponentNameURL}/${modifiedComponentNameURL}.html`;
			}
			setTimeout(function () {
				iframe.classList.remove("fade-out");
			}, 500);
